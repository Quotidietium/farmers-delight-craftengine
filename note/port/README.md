...
| 项 | 状态 |
|---|---|
| 炉灶滋滋环境音 | 已实现：点燃时随机间隔（均值 80t，REF 参数 60-100t / vol 1.0 / pitch 0.9~1.1）播放 crackle |
| 堆肥比较器 | 已实现：`farmersdelight:comparator_signal` 行为，mod 公式（信号 = 8-stage，随堆肥推进递减） |
| 砧板比较器 | 误报澄清：**mod 砧板本就无比较器输出**（REF 配置同样显式 has_comparator: false），无需移植 |
...
### ❓ 待用户决策：家具→CE 方块架构迁移（比较器输出的唯一路径）

mod 中烹饪锅/盛宴/派有比较器输出（锅=9 槽填充率公式，盛宴=servings，派=bites）。
Bukkit/Paper **没有任何 API** 能让 item_display 家具响应比较器查询（信号由客户端读取
方块状态产生，服务端无事件钩子）；REF 能实现是因为其锅/砧板/炉灶从一开始就是 CE 方块
+BlockEntityController 架构。我方对应方块为家具（item_display 实体），唯一实现路径是
把锅/盛宴/派迁移到 CE 方块架构（区块 PDC 存储 + 交互/掉落/显示全部重写，估 600-900 行
改动 + 全量回归）。**请确认**：① 接受迁移成本做完全复现；② 关闭为永久偏差（推荐：
比较器属红石小众用法，迁移会引入全量回归风险）。
